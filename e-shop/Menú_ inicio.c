...
{
	//Estas dos líneas de código son necesaria para poder imprimir la fecha
	time_t t = time(NULL);
    struct tm tm = *localtime(&t);
	int m0,m1,g;
	int m,j=0,k=0,n;
...
pf9 = fopen("historial_compras.txt", "a");//Abrimos el fichero para escribir sobre él
FILE *pf10;//puntero que señala a fichero 
...
//PARA SABER QUE NÚMERO DE COMPRA ES
do
 {
 m=strcmp(nick_usuario,compras[T].usuario);//bucamos los productos del usuario
 if(m==0)//si coinciden imprimimos los productos
 {
 strcpy(val,compras[T].n_compra);//si la compra es del usuario asigno el número de compra a val
 T++;//pasamos a la línea siguiente
}
 else 
 T++;//si la línea no es del usuario, paso a la siguiente
}while(T!=j);//el bucle termina cuando hayamos recorrido todo el fichero

*val+=1;//este es el número de compra que realizaremos ahora
//printf("%s\n",val);
//como el i me da el número de productos que he comprado,recorro los vectores conla compra i veces
for(T=0;T<i;T++)
{
fprintf(pf9,"%s;",nick_usuario);//imprimo el nick del usuario
fprintf(pf9,"%s;",val);//Imprimo número de compra es
fprintf(pf9,"%i;",i);//y te da el número de productos de la compra
fprintf(pf9,"%d/%d/%d;", tm.tm_mday, tm.tm_mon + 1, tm.tm_year + 1900);//imprimo la fecha
fprintf(pf9,"%s;",CODIGOS[T].CODIGOCOMPRA);//imprimo el código del producto
...
case 'g'://imprimimos en el fichero lista_provisional la lista
	{
	flag1=1;
	 for(T=0;T<i;T++)//i es el número de productos
	 {
	 fprintf(pf10,"%s;",nick_usuario);//imprimo el nick del usuario
     fprintf(pf10,"%i;",i);//y te da el número de productos de la compra
...
}while(flag1!=1);//sirve para que si el usuario presiona un caracter no válido como opción, el programa todavía continue
...
{//Función que filtra el catálogo de manera que solo muestre los objetos que pertenezcan a un intervalo de precios a elegir por el usuario.
...
{
m=fscanf(pf9,"%[^;];%[^;];%[^;];%[^;];%[^;];%i\n",compras[i].usuario,compras[i].n_compra,compras[i].n_prod,compras[i].fecha,CODIGOS[i].CODIGOCOMPRA,&unidades[i]);//Leemos y guardamos la información
J++;//contador de líneas del fichero
i++;
}
*p=J;
...
void historial_completo(char nick_usuario[],int dim)
{
	//Declaración de variables y tipos de variables a utilizar
 ...
 printf("\t\t\t\t\tFECHA: %s\n",compras[i].fecha);
 printf("\tProductos\t\t\t\t\tUnidades\t\t\t\t\tPrecio\n\n");//imprimimos los productos en pantalla según los códigos
 val=atof(compras[i].n_prod);//atof es una función que transforma una cadena de caracteres en un float. Transformo
 //el número de productos de cada compra a realya que inicialmente lo guarde como cadena de caracteres. Lo guardamos como cadena de caracteres para podre almacenar
 // esta información en un vector de estructuras del tipo historial(Lo puede ver en el fichero que guarda las estructuras) directamente utilizando un solo
 // fscanf(al leer el fichero historial_compras), ya que scanf no nos permitía leer cadenas de caracteres y números mezclados para cada línea del fichero
...
void historial_fechas(char nick_usuario[],int dim)
{
	//Declaración de varianles y tipos de variables a utilizar
 	        ...
 	        if(strcmp(FECHA,compras[i].fecha)==0)//comparamos la fecha con la fecha que quiero, si coincide, se imprimira toda la compra de esa fecha
          	{
	             ...
	             m=strcmp(productos[g].codigo,CODIGOS[i].CODIGOCOMPRA);//imprimimos los productos en pantalla comparando los códigos del historial con los codigos de todo el catálogo
	             }while(m!=0);
...
void ver_lista_provisional(char nick_usuario[],int dim)
{
   ...
   {
     m=fscanf(pf10,"%[^;];%[^;];%[^;];%[^;];%i\n",lista[j].usuario,lista[j].n_prod,lista[j].fecha,CODIGOS1[j].CODIGOCOMPRA,&unidades1[j]);//Leemos y guardamos la información
     j++;//avanzamos en el vector en donde guardamos la información de cada línea del fichero
     p++;//contador de líneas
   }

   fclose(pf10); // Cerramos fichero

   for(n=0;n<p;n++)//recorremos todos los elementos de los vectores en donde guardamos la información del fichero
   {
    m=strcmp(nick_usuario,lista[n].usuario);//bucamos los productos del usuario
    if(m==0)//si coinciden 
    {
 	   //Asignamos lainformación a  nuevos vectores que luego pasaremos como argumentos de entrada a la función mostrar_carrrito
 	   t=n;
 	   strcpy(CODIGOS[i].CODIGOCOMPRA,CODIGOS1[n].CODIGOCOMPRA);//.La asignación es de derecha a izquierda
 	   unidades[i]=unidades1[n];
       i++;//i tiene el número de productos de la lista
    }
   }
   
   n=t-i+1;//Este es el elemento en el que comienza la lista provisional del usuario

   //una vez ya leída, la lista deja de ser provisional y toda la información de la lista debe desaparecer del fichero, para dar espacio a otra futura lista provisional del usuario
   //desplazamos la información que iba despues de está lista i líneas más abajo
    for(j=0;j<=p-n+i;j++)
    {
    	strcpy(lista[n+j].usuario,lista[n+i+j].usuario);//A la posiciona n va el producto que estaba en la posición n+i
    ...
    //Imprimo la informacion de las listas provisionales (de todos los usuarios)en el fichero. Ya habiendo quitado la información de la lista del usuario 
    for(j=0;j<i;j++)
    {
    fprintf(pf10,"%s;",nick_usuario);//imprimo el nick del usuario
    fprintf(pf10,"%s;",lista[j].n_prod);//Imprimo número de compra es
    fprintf(pf10,"%i;",lista[j].fecha);//y te da el número de productos de la compra
    fprintf(pf10,"%s;",CODIGOS1[j].CODIGOCOMPRA);//imprimo el código del producto
    fprintf(pf10,"%i\n",unidades[j]);//imprimo las unidades
    }

  //LLamo a la función
  mostrar_carrito(CODIGOS,100,unidades,100,i);//i es el número de productos de la lista
}
