...
//ERROR, LA CONTRASE—A NO PUEDE ESCRIBIRSE, NO SALTA DE LÕNEA AL PULSAR ENTER.
    /*while(persona1.password[i]<17 && persona1.password[i]!='\n'){
        //Capturamos car·cter 
        persona1.password[i]=getch(); 
		if (getch()=='\n') persona1.password[i]='\0';  
		...
		for(i=0;i<17;i++){
			scanf(" %c",persona1.password[i]);
			if (persona1.password[i]== '\n'){
				persona1.password[i]= '\0';
				break;	
			}
	...
	//ComprobaciÛn de mayorÌa de edad
...
void login (char *p)
{
	...
	//Cadenas de caracteres para el nick y la contraseÒa del usuario logueandose
	char nick[16];
	char pass[16];
	...
	do
	{
		...
		//comprueba si el inicio de sesiÛn es correcto.
	
		while(fscanf(pf,"%[^;];%[^;];%[^;];%[^\n]\n",registrados[n].nombre,registrados[n].apellidos,registrados[n].nickname,registrados[n].password)!=EOF&&flag!=0)
		{
			//Comprobamos usuario y contraseÒa
