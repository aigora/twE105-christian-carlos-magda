...
void catalogo_completo()
{
	...
	for(k=0;k<4;k++)
        {
           if(*productos[i].Especificaciones[k].Descripcion==0)//Doble igual de pregunta, no de asignaciÛn//productos[0].Especificaciones[3].todo esto es Descripcion es un pruntero y aunque ponga *()no me da el valor
           m++;
        }
    //Doble igual de pregunta, no de asignaciÛn//printf("%s\n\tEspecificaciones:%s\n\tCodigo:%s\n"    
    if(m==0)*/
	printf("%s\n\tEspecificaciones:\n\t\t%s\n\t\t%s\n\t\t%s\n\tCodigo: %s\n",
	       productos[i].nombre,
